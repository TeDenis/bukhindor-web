      ...
      if (_liquidController.value % 0.1 < 0.005) {
        setState(() {
          if (_chaoticBubbles.length < 40) {
            // Увеличиваем лимит пузырьков
            final screenWidth = MediaQuery.of(context).size.width;
            final newBubble = ChaoticBubble(
              x: 20.0 +
                  _random.nextDouble() *
                      (screenWidth -
                          40.0), // Распределяем по всей ширине экрана
              y: MediaQuery.of(context).size.height,
              size: 4.0 +
                  _random.nextDouble() *
                      6.0, // Увеличиваем размер для лучшей видимости
              speed: 20.0 +
                  _random.nextDouble() *
                      60.0, // Больше вариативности в скорости
              horizontalDrift: (_random.nextDouble() - 0.5) *
                  30.0, // Больше горизонтального движения
              opacity: 0.7 +
                  _random.nextDouble() * 0.3, // Увеличиваем непрозрачность
                    ...
                    child: SlideTransition(
                      ...
                      child: Align(
                        ...
                        child: AnimatedContainer(
                          ...
                          curve: Curves.easeInOut,
                          height: _hasValidationErrors
                              ? 600
                              : 500, // Динамическая высота
                          child: Container(
    ...
    for (final bubble in chaoticBubbles) {
      ...
      if (bubble.y <= popHeight) {
        ...
        final popPaint = Paint()
          ..color =
              Colors.white.withOpacity(0.9 * popOpacity) // Увеличиваем яркость
        ...
        final bubblePaintDynamic = Paint()
          ..color = Colors.white
              .withOpacity(bubble.opacity * 0.8) // Увеличиваем яркость
          ..style = PaintingStyle.fill;
